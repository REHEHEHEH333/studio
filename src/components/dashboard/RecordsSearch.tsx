      ...
      <CardContent className="flex-grow flex flex-col">
        ...
        <Tabs defaultValue="individuals" className="flex-grow flex flex-col">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="individuals"><User className="h-4 w-4 mr-2"/>Individuals</TabsTrigger>
                  ...
                  {loading ? (
                    ...
                    individuals.map(ind => (
                      <TableRow key={ind.id}>
                        ...
                        <TableCell>{ind.address}</TableCell>
                        <TableCell><Badge variant={getLicenseStatusVariant(ind.license_status)}>{ind.license_status}</Badge></TableCell>
                      </TableRow>
                    ))
                  ) : (
                    <TableRow><TableCell colSpan={4} className="text-center">{searched ? 'No results found' : 'Enter a query to search'}</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </div>
